...
class GridTrading:
    """
    Implements grid trading strategy for the Elysium Trading Platform.
    Grid trading places orders at regular price intervals within a range,
    allowing traders to profit from price oscillations.
    """
    ...
    def start_grid(self, grid_id: str) -> Dict[str, Any]:
        ...
        with self.grid_lock:
            ...
            try:
                ...
                # This is just an estimate - let the order handler handle the exact formatting
                quantity_per_grid = grid["investment_per_grid"] / current_price
                
                self.logger.info(f"Calculated approximate quantity per grid: {quantity_per_grid}")
                
                # Place grid orders - use order handler's methods directly
                for price in grid_levels:
                    try:
                        # Buy orders for prices below current price
                        if price < current_price:
                            # Use the order handler's methods directly - they handle formatting properly
                            if grid["is_perp"]:
                                order_result = self.order_handler.perp_limit_buy(
                                    grid["symbol"], quantity_per_grid, price, grid["leverage"]
                                )
                            else:
                                order_result = self.order_handler.limit_buy(
                                    grid["symbol"], quantity_per_grid, price
                                )
                            ...
                            if order_result["status"] == "ok":
                                ...
                                if "response" in order_result and "data" in order_result["response"] and "statuses" in order_result["response"]["data"]:
                                    ...
                                    if "resting" in status:
                                        ...
                                        self.logger.info(f"Placed grid buy order at {price}")
                            else:
                                self.logger.error(f"Failed to place buy order at {price}: {order_result.get('message', '')}")
                        
                        # Sell orders for prices above current price
                        elif price > current_price:
                            # Use the order handler's methods directly - they handle formatting properly
                            if grid["is_perp"]:
                                order_result = self.order_handler.perp_limit_sell(
                                    grid["symbol"], quantity_per_grid, price, grid["leverage"]
                                )
                            else:
                                order_result = self.order_handler.limit_sell(
                                    grid["symbol"], quantity_per_grid, price
                                )
                            ...
                            if order_result["status"] == "ok":
                                ...
                                if "response" in order_result and "data" in order_result["response"] and "statuses" in order_result["response"]["data"]:
                                    ...
                                    if "resting" in status:
                                        ...
                                        self.logger.info(f"Placed grid sell order at {price}")
                            else:
                                self.logger.error(f"Failed to place sell order at {price}: {order_result.get('message', '')}")
                    except Exception as e:
                        self.logger.error(f"Error placing grid order at price {price}: {str(e)}")
                
                # Update grid with orders
                grid["orders"] = buy_orders + sell_orders
                grid["active"] = True
                grid["status"] = "active"
                
                self.logger.info(f"Started grid {grid_id} with {len(buy_orders)} buy orders and {len(sell_orders)} sell orders")
                ...
                return {
                    "status": "ok", 
                    "message": f"Grid {grid_id} started successfully",
                    "warning": warning_msg,
                ...
                with self.grid_lock:
                    ...
                    for order in grid["orders"]:
                        if order["status"] == "open" and order["id"] not in open_order_ids:
                            ...
                            try:
                                if order["side"] == "buy":
                                    # Place a sell order one grid level above
                                    sell_price = order["price"] + grid["price_interval"]
                                    
                                    # Use order handler's methods directly
                                    if grid["is_perp"]:
                                    ...
                                    if result["status"] == "ok":
                                        ...
                                        if "response" in result and "data" in result["response"] and "statuses" in result["response"]["data"]:
                                            ...
                                            if "resting" in status:
                                                new_order_id = status["resting"]["oid"]
                                                new_order = {
                                                    "id": new_order_id,
                                                    "price": sell_price,
                                                ...
                                                grid["orders"].append(new_order)
                                                self.logger.info(f"Grid {grid_id}: Placed new sell order at {sell_price}")
                                
                                elif order["side"] == "sell":
                                    # Place a buy order one grid level below
                                    ...
                                    if result["status"] == "ok":
                                        ...
                                        if "response" in result and "data" in result["response"] and "statuses" in result["response"]["data"]:
                                            ...
                                            if "resting" in status:
                                                ...
                                                grid["orders"].append(new_order)
                                                self.logger.info(f"Grid {grid_id}: Placed new buy order at {buy_price}")
                            
                            except Exception as e:
                                self.logger.error(f"Error placing new order after fill in grid {grid_id}: {str(e)}")
